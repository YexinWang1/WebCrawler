...
def extract_next_links(url, resp):
    ...
    global longest_url, longest_url_count, all_unique_urls, all_word_freq, all_subdomains, all_subdomain_urls

    # ------------------ 1. update the report -------------------------------------------------------
    
    if not (200 <= resp.status < 300 and resp.status != 204):  
        return [] # since status code 204 is No Content
...
def is_valid(url):
    ...
    try:
        # case 1
        if url == None or len(url) < 1 or len(url) > 100 or url == '#':
            return False
        ...
        flag = not re.match(
            ...
            + r"|thmx|mso|arff|rtf|jar|csv|bib|txt|odc|scm|py|ppsx"
        ...
        # case 5: avoid infinite loop
        if len(url) > 20:
            sub_parts = url.split('/')

            temp_dict = {}
            for sub_part in sub_parts:
                if sub_part not in temp_dict:
                    temp_dict[sub_part] = 1
                else:
                    temp_dict[sub_part] = temp_dict[sub_part] + 1
            
            for sub_part, count in temp_dict.items():
                if count >= 3:
                    return False
...
def tokenize(TextFilePath):
    ...
    for line in file:
        ...
        for word in line.split(' '):
            if word != '\n' and word != ' ' and len(word) >= 2 and word != 're' and word != 've' and word != 'll':
